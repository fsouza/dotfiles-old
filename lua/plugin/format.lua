...
local format_stdin = function(format_cmd, format_args, timeout_ms)
  ...
  if not status then
    error(string.format('%s did not complete in %dms, consider increasing the timeout', format_cmd,
                        timeout_ms))
...
function M.dune()
  format_stdin('dune', {'format-dune-file'})
end

function M.prettier()
  format_stdin('npx', {'prettier'; '--stdin-filepath'; vim.fn.expand('%p')})
end

function M.lua()
  format_stdin('lua-format', {})
end
